...
AI-модерация спама — Google Gemini.
...
Папка `./backups/` подмонтирована на хост — её можно отдельно архивировать/копировать на другой сервер через `rsync`/`scp`.
Если в будущем понадобится загрузка в S3 — добавить переменные `BACKUP_S3_*` и второй контейнер с `rclone`.

## Этапы

MVP в этом репозитории — скелет всех частей: модели, API, парсеры hh/SuperJob/Avito/Rabota.ru, AI-модерация, бот с онбордингом и расписанием рассылок, WebApp с лентой/деталями/фильтрами/сохранёнными/настройками + лендинг работодателей. ЮКасса — заглушка.
